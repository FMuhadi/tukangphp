...
class mpompa extends CI_Model {
    ...
    public function select($arrfilter,$url=""){
        ...
        $table .=" <tr><th>pompaid</th><th>namalokasi</th><th>r_kecamatanid</th><th>r_kelurahanid</th><th>titikkoordinat</th><th>jumlah</th><th>kapasitas</th><th>kondisi</th><th>keberfungsian</th><th>Action</th></tr>";
        foreach ($query->result() as $row)
        {
            $table .= "<tr>"."<td>".$row->pompaid."</td>"."<td>".$row->namalokasi."</td>"."<td>".$row->r_kecamatanid."</td>"."<td>".$row->r_kelurahanid."</td>"."<td>".$row->titikkoordinat."</td>"."<td>".$row->jumlah."</td>"."<td>".$row->kapasitas."</td>"."<td>".$row->kondisi."</td>"."<td>".$row->keberfungsian."</td>"."<td><a class='btn' href=\"//".base_url()."pompa\\edit\\".$row->pompaid."\">Edit</a> | <a class='btn btn-dangger' href=\"//".base_url()."pompa\\delete\\".$row->pompaid."\">Delete</a>".($url!=''?" | <a class='btn btn-dangger'  href='//".base_url()."$url/".$row->pompaid."'>Detail</a>":'')."</td></tr>";
        }
        $table .="</table>";
        return $table;
    }
    ...
    public function delete($id){
        $this->db->where('pompaid', $id);
        $this->db->delete('pompa');
    }
...
<div class='form-check'>
  ...
  <label class='form-check-label' for='keberfungsian'>
    ...
    public function update($data,$id){
        $this->db->where('pompaid', $id);
        $this->db->update('pompa', $data);
    }

    public function count(){
        return $this->db->count_all_results('pompa'); 
    }

    public function selectoption($filter=""){
        $this->db->select('pompaid,pompaname');
        $this->db->from('pompa');
        $query = $this->db->get();
        $form = "<div class='form-group'>
    <label for='r_pompaid' class='col-sm-2 control-label'>pompa</label>
    <div class='col-sm-10'>
      <select id='r_pompaid' class='form-control' name='r_pompaid' >";
        foreach ($query->result() as $row){
            $form.="<option value='".$row->pompaid."'>".$row->pompaname."</option>";
        }
        $form .="</select>
    </div>
  </div>";
